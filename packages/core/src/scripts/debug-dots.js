// Local debugging script: take base.jpg, produce result.jpeg with connect-the-dots rendering
// Usage:
//   node scripts\\debug-dots.js [--input path] [--output path] [--points N] [--threshold N]
// Defaults: input=./base.jpg, output=./result.jpeg, points=50, threshold=180
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === '--points' || a === '-p')
      args.points = parseInt(argv[++i], 10) || args.points;
    else if (a === '--threshold' || a === '-t')
      args.threshold = parseInt(argv[++i], 10) || args.threshold;
    else if (a === '--help' || a === '-h') {
      console.log(
        'Usage: node scripts\\debug-dots.js [--input path] [--output path] [--points N] [--threshold N]',
      );
      process.exit(0);
    }
  }
...
async function main() {
  ...
  const { input, output, points, threshold } = parseArgs(process.argv);
  const inPath = path.resolve(cwd, input);
  const outPath = output
    ? path.resolve(cwd, output)
    : path.join(path.dirname(inPath), 'result.jpeg');
  ...
  const svgPath = path.join(
    outDir,
    path.basename(outPath).replace(/\.[^.]+$/, '') + '.svg',
  );
