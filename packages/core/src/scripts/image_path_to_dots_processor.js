...
async function preprocessToBW(
  inputPath,
  tmpPath,
  { width = 1000, threshold = 180, blurSigma = 1.4 } = {},
) {
...
function traceWithPotrace(
  bwPath,
  { turdSize = 100, optCurve = true, alphaMax = 1.0, despeckle = true } = {},
) {
...
function pointsToSVGPage(
  points,
  {
    title = 'Соедини по точкам',
    width = 1000,
    height = 1414,
    pageNum = 1,
    showNumbering = true,
  ...
  const circles = points
    .map(([x, y]) => `<circle cx="${x}" cy="${y}" r="6.5" fill="#000"/>`)
    .join('\n');
  const numbers = showNumbering
    ? points
        .map(
          ([x, y], i) =>
            `<text x="${x + 10}" y="${y - 12}" font-family="Arial, sans-serif" font-size="16" fill="#111">${i + 1}</text>`,
        )
        .join('\n')
    : '';
...
function pointsToSVGMulti(
  paths,
  { width = 1000, height = 1414, pageNum = 1 } = {},
) {
  const header = `
  <rect x="16" y="16" width="${width - 32}" height="${height - 32}" rx="18" ry="18" fill="none" stroke="#222" stroke-width="3"/>
  <text x="40" y="60" font-family="Arial, sans-serif" font-size="28">Имя: ____________________________</text>
  ...
  const pathEls = all
    .map(
      (p) =>
        `<path d="${p.d}" fill="none" stroke="#bbb" stroke-width="1.5" stroke-dasharray="6 8" />`,
    )
    .join('\n');
...
async function imageToDots({
  ...
  if (!dList.length)
    throw new Error('Не найдены контуры на входном изображении.');
  ...
  const coreSubpaths = allSubpaths.filter(
    (d, i) => Math.abs(areas[i]) >= decorThreshold,
  );
  ...
  if (!multiContours) {
    ...
    let bestIdx = 0;
    for (let i = 1; i < coreSubpaths.length; i++)
      if (areas[i] > areas[bestIdx]) bestIdx = i;
    ...
    list.sort(
      (a, b) => Math.abs(pathAreaApprox(b)) - Math.abs(pathAreaApprox(a)),
    );
  ...
  if (pointsDistribution === 'equal') {
    ...
    let j = 0;
    while (rest-- > 0) {
      alloc[j % mainSubpaths.length]++;
      j++;
    }
  } else {
    for (let i = 0; i < mainSubpaths.length; i++)
      alloc.push(
        Math.max(1, Math.floor((lengths[i] / totalLength) * pointsCount)),
      );
    ...
    while (delta !== 0) {
      if (delta > 0) {
        alloc[k % alloc.length]++;
        delta--;
      } else {
        if (alloc[k % alloc.length] > 1) {
          alloc[k % alloc.length]--;
          delta++;
        }
      }
  ...
  const resultPaths = mainSubpaths.map((d, i) =>
    simplifyPoints(resamplePathD(d, alloc[i]), simplifyTolerance),
  );
  ...
  const scaledPaths = resultPaths.map((pts) =>
    pts.map(([x, y]) => [x * scale + ox, y * scale + oy]),
  );
  ...
  if (numbering === 'per-contour') {
    ...
    for (const pts of scaledPaths) {
      for (let i = 0; i < pts.length; i++)
        numbered.push([pts[i][0], pts[i][1], globalIdx++]);
    }
  } else {
    // continuous
    let idx = 1;
    for (const pts of scaledPaths)
      for (const p of pts) numbered.push([p[0], p[1], idx++]);
  }

  // 9) Выведем страницу SVG
  const dPaths = scaledPaths.map(pointsToSVGPath);
  const circles = numbered
    .map(([x, y]) => `<circle cx="${x}" cy="${y}" r="6.5" fill="#000"/>`)
    .join('\n');
