...
const {
  ...
  NAME_BY_KEY,
  GENERATORS,
} = require('../src/scripts/generate-custom.js');

function tmpDir() {
  const p = path.join(
    os.tmpdir(),
    `wg-core-custom-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  return p;
}
...
test('generateCustom throws on empty tasks', () => {
  assert.throws(
    () => generateCustom({ tasks: [] }),
    /Не выбрано ни одного вида задания/,
  );
});

test('generateCustom throws on unknown task', () => {
  assert.throws(
    () => generateCustom({ tasks: ['unknown-task'] }),
    /Неизвестные типы заданий/,
  );
});

test('generateCustom creates files and index for maze task', () => {
  const out = tmpDir();
  const res = generateCustom({
    days: 1,
    tasks: ['maze'],
    outRoot: out,
    seed: 'unit',
...
test('NAME_BY_KEY contains entries for known generators', () => {
  for (const key of Object.keys(GENERATORS)) {
    assert.ok(
      Object.prototype.hasOwnProperty.call(NAME_BY_KEY, key),
      `NAME_BY_KEY should have ${key}`,
    );
    assert.ok(
      typeof NAME_BY_KEY[key] === 'string' && NAME_BY_KEY[key].length > 0,
    );
  }
});
