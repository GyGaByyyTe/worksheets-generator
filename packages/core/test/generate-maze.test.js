...
test('generateMazePage returns SVG with expected header and content', () => {
  const svg = generateMazePage({ rows: 8, cols: 8, margin: 10, seed: 'unit', pageNum: 3, theme: { start: 'S', finish: 'F' } });
  assert.equal(typeof svg, 'string');
  assert.ok(svg.startsWith('<svg'), 'starts with <svg');
  assert.ok(svg.includes('ЛАБИРИНТ'), 'contains title');
  assert.ok(svg.includes('Проведи путь от S к F.'), 'contains subtitle with theme');
