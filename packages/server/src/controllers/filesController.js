...
async function downloadGeneration(req, res) {
  try {
    ...
    if (!files || files.length === 0) return res.status(404).json({ error: 'no pages found' });

    const imgs = files
      .map((f) => `    <img class=\"page\" src=\"/files/${f.id}\" alt=\"${f.filename}\" />`)
      .join('\n');
    ...
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=\"worksheets-${genId}.pdf\"`);
      return res.send(pdf);
    } catch (e) {
      // Fallback to HTML attachment if PDF generation is unavailable
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      res.setHeader('Content-Disposition', `attachment; filename=\"worksheets-${genId}.html\"`);
      return res.send(html);
    }
  } catch (e) {
    return res.status(400).json({ error: String((e && e.message) || e) });
