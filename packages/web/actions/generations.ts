...
export const getRecentGenerations = cache(
  async function getRecentGenerations(opts?: {
    limit?: number;
    mine?: boolean;
    token?: string | null;
    ...
    const url = absUrl(
      `/generations/recent?limit=${encodeURIComponent(String(limit))}&mine=${mine}`,
    );
