...
export default function ImageDotsTable({ rows, setRows, lockedCount = null }: ImageDotsTableProps) {
  ...
  React.useEffect(() => {
    ...
    if (rows.length < lockedCount) {
      const add = Array.from({ length: lockedCount - rows.length }, () => defaultParams());
      setRows([...rows, ...add]);
    } else {
  ...
  return (
    <div className="panel" style={{ marginTop: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        {lockedCount == null && (
          <div>
            <button type="button" onClick={addRow} style={{ marginRight: 6 }}>Добавить картинку</button>
          </div>
        )}
      </div>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i}>
                ...
                {lockedCount == null && (
                  <td style={tdCenter}>
                    <button type="button" onClick={() => removeRow(i)}>Удалить</button>
                  </td>
                )}
              </tr>
            ))}
            {rows.length === 0 && (
              <tr>
                <td style={td} colSpan={lockedCount == null ? 13 : 12}>Нет картинок. Используйте кнопку выше, чтобы добавить.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div style={{ marginTop: 8, color: '#555' }}>
        Подсказка: если включить привязку количества картинок к числу дней, строк будет ровно столько, сколько выбрано дней.
      </div>
    </div>
  );
}

const th: React.CSSProperties = { textAlign: 'left', padding: '6px 8px', borderBottom: '1px solid #ddd', fontWeight: 600, fontSize: 12 };
