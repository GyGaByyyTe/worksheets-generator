...
function pageConnectDots(pageNum) {
  ...
  const fish1 = [
    [0.1,0.5],[0.25,0.35],[0.5,0.3],[0.78,0.45],[0.85,0.4],[0.92,0.3],[0.95,0.5],[0.92,0.7],[0.85,0.6],[0.78,0.55],[0.5,0.7],[0.25,0.65]
  ];
  const fish2 = [
    [0.12,0.5],[0.28,0.38],[0.48,0.32],[0.66,0.38],[0.78,0.48],[0.86,0.42],[0.92,0.34],[0.94,0.5],[0.92,0.66],[0.86,0.58],[0.78,0.52],[0.66,0.62],[0.48,0.68],[0.28,0.62]
  ];
  const fish3 = [
    [0.15,0.5],[0.3,0.4],[0.5,0.35],[0.7,0.45],[0.82,0.5],[0.9,0.42],[0.92,0.5],[0.9,0.58],[0.82,0.5],[0.7,0.55],[0.5,0.65],[0.3,0.6]
  ];
  const cat1 = [
    [0.2,0.75],[0.2,0.45],[0.3,0.3],[0.4,0.45],[0.5,0.3],[0.6,0.45],[0.7,0.3],[0.8,0.45],[0.8,0.75],[0.6,0.8],[0.5,0.7],[0.4,0.8]
  ];
  const cat2 = [ // сидящий кот (силуэт упрощённый)
    [0.25,0.75],[0.3,0.55],[0.35,0.42],[0.42,0.38],[0.5,0.42],[0.52,0.35],[0.58,0.32],[0.64,0.38],[0.66,0.48],[0.65,0.58],[0.6,0.68],[0.52,0.75],[0.42,0.78],[0.32,0.78]
  ];
  const cat3 = [ // кот в профиль с хвостом
    [0.22,0.72],[0.28,0.6],[0.36,0.52],[0.46,0.5],[0.52,0.46],[0.56,0.38],[0.6,0.32],[0.66,0.34],[0.64,0.42],[0.6,0.5],[0.58,0.58],[0.5,0.66],[0.4,0.7],[0.3,0.72]
  ];
  const bird1 = [ // птица с расправленными крыльями
    [0.12,0.55],[0.22,0.48],[0.3,0.44],[0.38,0.47],[0.46,0.52],[0.5,0.56],[0.54,0.52],[0.62,0.47],[0.7,0.44],[0.78,0.48],[0.88,0.55],[0.76,0.52],[0.64,0.54],[0.5,0.62],[0.36,0.54],[0.24,0.52]
  ];
  const bird2 = [ // птица в полёте (наклон)
    [0.18,0.6],[0.3,0.5],[0.42,0.44],[0.5,0.46],[0.6,0.5],[0.68,0.54],[0.78,0.6],[0.7,0.58],[0.6,0.6],[0.5,0.64],[0.4,0.62],[0.28,0.64]
  ];
  const bird3 = [ // птичка на ветке (овал+клюв)
    [0.3,0.6],[0.36,0.54],[0.45,0.52],[0.55,0.54],[0.62,0.6],[0.6,0.64],[0.55,0.66],[0.47,0.68],[0.38,0.66],[0.32,0.62],[0.26,0.6],[0.24,0.58],[0.32,0.58]
  ];
  const bird4 = [ // силуэт чайки
    [0.12,0.58],[0.22,0.5],[0.34,0.46],[0.46,0.48],[0.5,0.5],[0.54,0.48],[0.66,0.46],[0.78,0.5],[0.88,0.58],[0.76,0.54],[0.64,0.52],[0.5,0.56],[0.36,0.52],[0.24,0.54]
  ];
  const fish4 = [ // короткая рыба с широким хвостом
    [0.18,0.5],[0.35,0.4],[0.55,0.38],[0.7,0.46],[0.78,0.5],[0.86,0.42],[0.92,0.34],[0.94,0.5],[0.92,0.66],[0.86,0.58],[0.78,0.5],[0.7,0.54],[0.55,0.62],[0.35,0.6]
  ];
  const fish5 = [ // каплевидная
    [0.2,0.5],[0.34,0.4],[0.52,0.36],[0.68,0.44],[0.78,0.5],[0.68,0.56],[0.52,0.64],[0.34,0.6]
  ];

  const BASES = [fish1, fish2, fish3, fish4, fish5, cat1, cat2, cat3, bird1, bird2, bird3, bird4];
