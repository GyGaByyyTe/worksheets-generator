...
function pointsToSVGMulti({
  ...
  for (let ci = 0; ci < contours.length; ci++) {
    ...
    const hint = pts
      ...
      .join(' L ');
    layers.push(
      `<path d="M ${hint}" fill="none" stroke="#bbb" stroke-width="1.5" stroke-dasharray="6 8" />`,
    );
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      dots.push(
        `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="6.5" fill="#000"/>`,
      );
      const k = i % dxAlt.length;
      const label =
        numbering === 'perContour' ? String(localNum++) : String(num++);
      texts.push(
        `<text x="${(x + dxAlt[k]).toFixed(1)}" y="${(y + dyAlt[k]).toFixed(1)}" font-family="Arial, sans-serif" font-size="16" fill="#111">${label}</text>`,
      );
    }
  }
...
function pointsToSVGPage(
  pts,
  { width = 1000, height = 700, margin = 50, labels = null },
) {
  const left = margin,
    top = margin;
  const w = width - margin * 2,
...
async function imageToDots({
  ...
  const tmpBW = path.join(
    os.tmpdir(),
    `dots-bw-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}.png`,
  );
  try {
    ...
    if (!dList.length)
      throw new Error(
        'Контур не найден. Попробуйте другой threshold или более контрастное изображение.',
      );

    const allDs = dList.flatMap((d) => splitSubpaths(d));

    const preBBoxes = allDs.map((d) =>
      getBBoxOfD(svgpath(d).unshort().unarc().abs().toString(), 240),
    );
    const globalBBox = preBBoxes.reduce(
      (g, b) => ({
    ...
    const items = allDs
      .map((d, idx) => {
        ...
        const minSide = Math.min(
          Math.max(1e-6, bbox.w),
          Math.max(1e-6, bbox.h),
        );
      ...
      .sort(
        (a, b) =>
          b.boxArea - a.boxArea || a.frameScore - b.frameScore || b.len - a.len,
      );
      ...
      if (targetContours && targetContours > 0) {
        for (const it of ranked) {
          const sameOrientation =
            (Math.sign(it.signedArea) || 1) === mainOrient;
          const notTooSmall =
            (it.boxArea || 0) > mainArea * Math.min(0.1, decorAreaRatio);
          const notThin =
            it === main ||
            it.thinScore >= THINNESS_LIMIT ||
            it.minSide >= ABS_MIN_SIDE ||
            it.boxArea > mainArea * 0.45;
          if (sameOrientation && notTooSmall && notThin) {
            contoursD.push(it.d);
            if (contoursD.length >= Math.min(targetContours, maxContours))
              break;
          } else {
            decorD.push(it.d);
          }
    ...
    const normalized = sampledContours.map((pts) =>
      rotateStartToBottomLeft(
        normalizeWithBBoxPreserveAR(pts, bbox, { fit: 'contain' }),
      ),
    );
